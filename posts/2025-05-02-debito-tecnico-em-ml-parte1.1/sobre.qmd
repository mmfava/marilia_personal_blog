---
title: "Devo mas não sei, pago quando puder?"
subtitle: "Dívida técnica oculta em sistemas de ML."
image: image.png
author:
...
1.  **Quão fácil é testar, em larga escala, uma nova abordagem algorítmica?** → Se o sistema está fortemente acoplado ou sem isolamento, qualquer substituição de modelo exige retrabalho estrutural — um sinal claro de dívida técnica.

2.  **Qual é o fechamento transitivo de todas as dependências de dados?** → Ou seja, até que ponto os dados usados por um modelo dependem de sinais indiretos ou fontes externas instáveis?

3.  **Quão bem conseguimos medir o impacto de uma mudança no sistema?** → Ausência de métricas, benchmarks históricos e rastreabilidade impede qualquer avaliação objetiva de regressões.

4.  **Uma melhoria em um modelo ou sinal tende a degradar outros?** → Isso indica interdependência não gerenciada — um forte indicador de dívida estrutural.

5.  **Com que rapidez um novo membro da equipe consegue contribuir de forma significativa?** → Tempo excessivo de onboarding pode sinalizar falta de documentação, ausência de padrões, complexidade acoplada ou conhecimento tácito demais.
